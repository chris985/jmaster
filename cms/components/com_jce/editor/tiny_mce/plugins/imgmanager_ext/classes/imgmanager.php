...
class WFImageManagerExtendedPlugin extends WFMediaManager {
    ...
    public function __construct() {
        ...
        if (JRequest::getCmd('dialog', 'plugin') == 'plugin') {
            ...
            if (JRequest::getInt('standalone') === 1) {
                ...
                $this->set('_edit', 0);
                $this->set('_standalone', 1);

                $params = $this->getParams();
                $browser->setFileTypes($params->get('browser.extensions', $this->get('_filetypes', 'images=jpg,jpeg,png,gif')));
            }
        } else {
    ...
    public function display() {
        ...
        if (JRequest::getCmd('dialog', 'plugin') == 'plugin') {
            ...
            // standalone mode
            if ($document->get('standalone', 0) === 1) {
                $document->addScript(array('browser'), 'component');
                $document->addScriptDeclaration('jQuery(document).ready(function($){$.WFBrowserWidget.init(' . json_encode($this->getSettings()) . ');});');
            // normal mode
            } else {
            ...
            $document->addScript(array('canvas', 'widget'), 'plugins');
            ...
            $document->addScriptDeclaration('jQuery(document).ready(function($){EditorDialog.init()});');
        }
    ...
    private function getImageEditor() {
        ...
        if (!is_object($editor)) {
            $editor = new WFImageEditor(array(
                'ftp'               => $this->isFtp(),
                'edit'              => $this->canEdit(),
                'prefer_imagick'    => (bool) $this->getParam('imgmanager_ext.prefer_imagick', true),
    ...
    public function onUpload($file, $relative = '', $method = '') {
        ...
        // override input if option is not avaialble
        if (!$params->get('upload_watermark', 0)) {
          $watermark = $params->get('upload_watermark_state', 0);
        }
        ...
        if ($method != 'inline') {
            ...
            if ((bool) $params->get('upload_resize', 1)) {
                ...
                $rw = (int) JRequest::getInt('upload_resize_width');
                $rh = (int) JRequest::getInt('upload_resize_height');
            }
        ...
        if ($dim) {
            ...
            if (!empty($exif['Orientation'])) {
                ...
                switch($orientation) {
                    case 3:
                        $rotate = $editor->rotate($file, 180);
                        break;
                    case 6:
                        $rotate = $editor->rotate($file, 90);
                        break;
                    case 8:
                        $rotate = $editor->rotate($file, 270);
                        break;
                }
                // no need to remove exif data on successful rotate
                if ($rotate) {
                    $removeExif = false;
                }
            }
            ...
            if ($watermark) {
                $font_style = $params->get('watermark_font_style', 'LiberationSans-Regular.ttf');
                // default LiberationSans fonts
                if (preg_match('#^LiberationSans-(Regular|Bold|BoldItalic|Italic)\.ttf$#', $font_style)) {
                    $font_style = WFUtility::makePath(WF_EDITOR_PLUGIN . '/fonts/', $font_style);
                // custom font
                } else {
                ...
                $options = array(
                    'type' => $params->get('watermark_type', 'text'),
                    'text' => $params->get('watermark_text', ''),
                    'image' => $params->get('watermark_image', ''),
                    'font_style' => str_replace(JPATH_SITE, '', $font_style),
                    'font_size' => $params->get('watermark_font_size', '32'),
                    'font_color' => $params->get('watermark_font_color', '#FFFFFF'),
                    'opacity' => $params->get('watermark_opacity', 50),
                    'position' => $params->get('watermark_position', 'center'),
                    'margin' => $params->get('watermark_margin', 10),
                    'angle' => $params->get('watermark_angle', 0)
                );
                ...
                if ($editor->watermark($file, $options)) {
                    ...
                    $browser->setResult(WFText::_('WF_IMGMANAGER_EXT_WATERMARK_ERROR'), 'error');
                }
            }

            if ($removeExif) {
                // remove exif data
                if (WFUtility::removeExifData($file) === false) {
                    @unlink($file);
    ...
    public function applyEdit($file, $task, $value) {
        ...
        if (isset($upload) && isset($upload['tmp_name']) && is_uploaded_file($upload['tmp_name'])) {
            ...
            // create image
            $image = new WFImage($upload['tmp_name'], array('preferImagick' => $this->getParam('prefer_imagick', true)));
    ...
    public function saveEdit($file, $name, $options = array(), $quality = 100) {
        ...
        if (isset($upload) && isset($upload['tmp_name']) && is_uploaded_file($upload['tmp_name'])) {
            ...
            require_once(dirname(__FILE__) . '/image/image.php');
            // create image
            $image = new WFImage($file, array('preferImagick' => $this->getParam('prefer_imagick', true)));
    ...
    public function getDefaults($defaults = array()) {
        ...
        $defaults = array(
            ...
            // watermark
            'upload_watermark' => $params->get('upload_watermark', 0),
            'upload_watermark_state' => $params->get('upload_watermark_state', 0)
        );
